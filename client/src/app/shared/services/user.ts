...
export class User {
  ...
  private teamLeadUsers$: Observable<IUser[]> | null = null;
  ...
  public teamLeadUsers(): Observable<IUser[]> {
    if (!this.teamLeadUsers$) {
      this.teamLeadUsers$ = this.api.get<ApiResponse<IUser[]>>(`${Constant.TEAM_LEAD_USER}`).pipe(
        map((response : ApiResponse<IUser[]>) => response.data),
        shareReplay(1)
      );
    }
    return this.teamLeadUsers$;
  }
}
